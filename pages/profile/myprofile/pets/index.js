...
export default function Pets({ petsList }) {
  ...
  if (session) {
    return (
      <SettingsLayout>
        <Head>
          <title>Mascotas | Sweet Home</title>
        </Head>
        <h1 className="title">Mis mascotas</h1>
        <div className="pet__buttons">
          <button className={global.buttonPrimary}>
            <a
              href="/profile/myprofile/pets/createPet"
              title="Ir a la página para añadir mascotas"
              aria-label="Ir a la página para añadir mascotas"
            >
              Añadir mascota
            </a>
          </button>
        ...
        {pets.length === 0 && (
          <div>
            <p className={global.loading2}>No tienes mascotas registradas.</p>
          </div>
        )}
        {pets.map(
          ({
