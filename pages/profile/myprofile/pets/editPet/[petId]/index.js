...
export default function EditPet ({pets}) {
  ...
  const editPet = async (e) => {
    ...
    if(name.trim() === ''){
      ...
      if (data.error) {
        ...
        toast.success('Se ha editado la mascota correctamente', { position: "bottom-right",
        autoClose: 5000,
        hideProgressBar: true,
        closeOnClick: true,
        pauseOnHover: true,
  ...
  if(session){
    return (
      <Layout>
        <Head><title>Editar mascota | Sweet Home</title></Head>
        ...
        <div className='editPet__form'>
            ...
            <form action='/api/pets' id='form'>
                ...
                <input className={global.buttonPrimary} type='submit' onClick={(e) => editPet(e)} value={isEditing ? 'Editando..' : 'Editar'} />
              </div>
            </div>
