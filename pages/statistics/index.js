...
export default function Statistics({ dataset }) {
  ...
  useEffect(() => {
    ...
    if (dataset != {}) {
      setBarData({
        labels: data?.numberPostsPerDay.map(
          (numberPostPerDay) => numberPostPerDay.date
        ),
        datasets: [
          {
            label: "Número de publicaciones por día",
            data: data?.numberPostsPerDay.map(
              (numberPostPerDay) => numberPostPerDay.count
            ),
      ...
      setVerticalBarData({
        labels: data?.mostPopularUsers.map(
          (mostPopularUser) => mostPopularUser.username
        ),
        datasets: [
          {
            label: "Seguidores",
            data: data?.mostPopularUsers.map(
              (mostPopularUser) => mostPopularUser.followersCount
            ),
            fill: true,
            backgroundColor: "#f0810f",
          },
      ...
      setRadarData({
        labels: data?.typeAttendances.map(
          (typeAttendance) => typeAttendance.name
        ),
        datasets: [
          {
            label: "Hilos",
            data: data?.typeAttendances.map(
              (typeAttendance) => typeAttendance.threads.length
            ),
            fill: true,
            backgroundColor: "#f0810f",
            borderColor: "#f0810f",
      ...
      setRadarOptions({
        ...
        plugins: {
          title: {
            display: true,
            text: "Tipos de cuidado con más hilos",
          },
        },
        maintainAspectRatio: true,
        aspectRatio: 2,
      ...
      setLineData({
        labels: data?.numberUsersPerDay.map(
          (numberUsersPerDay) => numberUsersPerDay.date
        ),
        datasets: [
          {
            fill: true,
            label: "Nuevos usuarios",
            data: data?.numberUsersPerDay.map(
              (numberUsersPerDay) => numberUsersPerDay.count
            ),
            borderColor: "#f0810f",
            backgroundColor: "#f0810f",
          },
      ...
      setPieData({
        labels: data?.numberPostsPerUser.map(
          (numberPostPerUser) => numberPostPerUser.username
        ),
        datasets: [
          {
            label: "Número de publicaciones",
            data: data?.numberPostsPerUser.map(
              (numberPostPerUser) => numberPostPerUser.postCount
            ),
            backgroundColor: [
              "rgba(255, 99, 132, 0.8)",
              "rgba(54, 162, 235, 0.8)",
  ...
  if (session?.user.role === "gerente") {
    return (
      <Layout>
        <Head>
          <title>Estadísticas | Sweet Home</title>
        </Head>
        <div className="statistics__header">
          <h1 className="title">Estadísticas de la aplicación</h1>
          <p className={global.text}>
            ¡Bienvenido al panel de estadísticas! A continuación, podrá acceder
            a representaciones visuales de los datos que se han almacenado en la
            aplicación.
          </p>
        </div>
        ...
        <div className="average__container">
          ...
          <div className="average__container__item">
            ...
            <h1 className="number__text">
              {data?.averageThreadsPerTypeAttendance}
            </h1>
            <p className={global.text2}>hilos</p>
          </div>
          <div className="average__container__item">
          ...
          <div className="average__container__item">
            ...
            <h1 className="number__text">
              {data?.averageAttendancesPerThread}
            </h1>
            <p className={global.text2}>cuidados</p>
          </div>
          <div className="average__container__item">
        ...
        <div className="statistics__postUser">
          ...
          <div className="user__card">
            ...
            <p className={global.date2}>
              {new Date(user?.createdAt).toLocaleDateString().slice(0, 10)}
            </p>
        ...
        <h2 className={global.title7}>Usuarios más populares</h2>
        ...
        <div className="statistics__popularUser">
          <h2 className={global.title7}>Usuarios más activos</h2>
          ...
          <div className={global.content}>
            <div className="message">
              <h1 className={global.title7}>
                Para acceder a esta página debe iniciar sesión como gerente
              </h1>
