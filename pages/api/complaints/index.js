...
export default async function handler (req, res) {
  ...
  if (req.method === 'DELETE') {
    ...
    await db.collection('complaints').updateOne({ _id: ObjectId(body.id) }, {$set: {isChecked: true, isApproved: false, adminId: admin._id}})
  ...
  if (req.method === 'PUT') {
    ...
    await db.collection('complaints').updateOne({ _id: ObjectId(body.id)}, {$set: {isChecked: true, isApproved: true, adminId: admin._id}})
