...
export default async function handler(req, res) {
  ...
  if (req.method == "PUT") {
    ...
    if (body.password != body.confirmPassword) {
      return res
        .status(400)
        .json({ message: "Ambas contraseñas no coinciden." });
    }
    ...
    jwt.verify(req.query.token, secret, function (err) {
      if (err) {
        if (err.name === "TokenExpiredError")
          ...
          return res
            .status(400)
            .json({ message: "Ha ocurrido un error al verificar el enlace." });
      } else {
    ...
    if (user) {
      ...
      await db
        .collection("users")
        .updateOne({ email: user.email }, { $set: { password: hashPassword } });
