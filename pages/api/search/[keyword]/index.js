...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    const postsByDescription = await db
      .collection("posts")
    ...
    const typePost = await db
      .collection("posts")
      .find({ "type.name": { $regex: keyword, $options: "i" } })
      .limit(50)
      .toArray();

    //Buscar publicaciones (según ubicación)

    const postsByLocation = await db
      .collection("posts")
    ...
    const attendancesByUser = await db
    ...
    const attendancesByDescription = await db
      .collection("attendances")
      .find({ description: { $regex: keyword, $options: "i" } })
      .limit(50)
      .toArray();
    ...
    const attendancesByBreed = await db
      .collection("attendances")
      .find({ breed: { $regex: keyword, $options: "i" } })
      .limit(50)
      .toArray();

    //Buscar cuidados  (según animal)

    const attendancesByAnimal = await db
      .collection("attendances")
      .find({ animal: { $regex: keyword, $options: "i" } })
      .limit(50)
      .toArray();

    // Buscar tipo de cuidado (por nombre)

    const typeAttendanceByTitle = await db
    ...
    const typeAttendanceByDescription = await db
      .collection("typeAttendance")
      .find({ description: { $regex: keyword, $options: "i" } })
      .limit(50)
      .toArray();
    ...
    const newsByIntroduction = await db
      .collection("news")
      .find({ introduction: { $regex: keyword, $options: "i" } })
      .limit(50)
      .toArray();

    //Buscar noticias relacionadas por contenido

    const newsByContent = await db
      .collection("news")
      .find({ body: { $regex: keyword, $options: "i" } })
      .limit(50)
      .toArray();

    //Buscar noticias relacionadas por conclusión

    const newsByConclusion = await db
      .collection("news")
      .find({ conclusion: { $regex: keyword, $options: "i" } })
      .limit(50)
      .toArray();

    //Buscar noticias relacionadas por autor

    const newsByAuthor = await db
      .collection("news")
      .find({ author: { $regex: keyword, $options: "i" } })
      .limit(50)
      .toArray();
