...
export default async function handler (req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const data = await db.collection('comments').insertOne({ description: body.description, username: body.username, userId: user._id, postId: ObjectId(body.postId), createdAt: new Date() })
    ...
    await db.collection('notifications').insertOne({_id: id, sender: user._id, receiver: postOwner.userId, type: typeNotification, 
      description: `@${body.username} te ha comentado una publicación`, isChecked: true, createdAt: new Date()})
