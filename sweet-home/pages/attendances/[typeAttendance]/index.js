...
export default function TypeAttendance({threads}){
    ...
    if(session){
        return(
            <Layout>
                <Head>
                    <title>Hilos de {router.query.typeAttendance}</title>
                </Head>
                <h1 className={global.title}>Hilos de {router.query.typeAttendance}</h1>
                ...
                {threads.length === 0 && <div><p className={global.loading}>Cargando..</p></div>}
