...
const CommentsCounter = dynamic(() =>
  import("/components/CommentsCounter/CommentsCounter")
);
...
/**
 * This function is a comment component that receive props from post and displays them
 * in a comment format created by user
 * @param props - props received from page.
 * @returns An comment created by user.
 */
export default function Comment(props) {
  ...
  const Commentate = async () => {
    if (comment.trim() === "") {
      toast.error("Debe escribir un comentario", {
    ...
    if (data?.error) {
      ...
      toast.success("Se ha publicado el comentario", {
        position: "bottom-right",
        autoClose: 5000,
        hideProgressBar: true,
        closeOnClick: true,
  ...
  const deleteComment = async (_id) => {
    ...
    toast.error("Se ha eliminado el comentario", {
      position: "bottom-right",
      autoClose: 5000,
      hideProgressBar: true,
      closeOnClick: true,
      ...
      <div className="comment__container">
        <div className="post__block">
          <div className="post__comment">
            ...
            <input
              type="submit"
              value={isComment ? "Enviando.." : "Enviar"}
              onClick={() => Commentate()}
        ...
        <div className="comment__title">
          <div className="title__start">
            ...
            <button className="refresh__button" onClick={() => getComments()}>
              <HiOutlineRefresh size={15} color={colors.quaternary} />
            </button>
          </div>
        ...
        {comments?.length === 0 && (
          <p className={global.text}>No hay ningún comentario</p>
        )}
        {comments?.length != 0 &&
              ...
              <>
                <div key={comment?._id} className={global.comment}>
                  ...
                  <div className="comment__date">
                    <p className={global.time}>
                      Hace {calcTime(comment?.createdAt)}
                    </p>
        ...
        {comments?.length > 3 && isVisible === false && (
          <button
            className="button__see"
            onClick={() => {
        ...
        {moreComments &&
          comments?.length != 0 &&
          comments?.slice(3, comments?.length).map((comment) => {
            return (
              <>
                <div key={comment?._id} className={global.comment}>
                  <div className="comment__username">
