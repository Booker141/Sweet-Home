...
/**
 * This function is a post component that receive props from page and displays them
 * in a post format created by user
...
export default function Post(props) {
  ...
  async function getUser() {
    ...
    if (data?.role.name === "protectora") setIsShelter(true);
    if (data?.role.name === "veterinaria") setIsVet(true);
    if (data?.role.name === "administrador") setIsAdmin(true);
    if (data?.role.name === "gerente") setIsManager(true);
  }
  ...
  const deletePost = async () => {
    ...
    toast.error(`Se ha eliminado la publicación`, {
        ...
        <div key={props._id} className={global.post}>
          <div className="post__header">
            <div className="header__user">
              ...
              <div className="user__info">
                <div className="info__username">
                  ...
                  {isShelter && <MdPets size={15} color={colors.secondary} />}
                  {(isAdmin || isManager) && (
                    <BsPatchCheckFill size={15} color={colors.secondary} />
                  )}
                  {isVet && (
                    <MdHealthAndSafety size={15} color={colors.secondary} />
                  )}
                </div>
            ...
            <div className="header__location">
              ...
              {props.type?.name === "Silvestre" && (
                <a
                  className={global.tag}
                  href={`${server}/wild`}
                  aria-label="Ir a página de fauna silvestre"
                >
                  #Silvestre
                </a>
              )}
              {props.type?.name === "Adopción" && (
                <a
                  className={global.tag}
                  href={`${server}/adoption`}
                  aria-label="Ir a página de animales para adoptar"
                >
                  #Adopción
                </a>
              )}
              {props.type?.name === "Perdido" && (
                <a
                  className={global.tag}
                  href={`${server}/lost`}
                  aria-label="Ir a página de animales perdidos"
                >
                  #Perdido
                </a>
              )}
              {props.type?.name === "Abandonado" && (
                <a
                  className={global.tag}
                  href={`${server}/abandoned`}
                  aria-label="Ir a página de animales abandonados"
                >
                  #Abandonado
                </a>
              )}
          ...
          <div className="description">
            <div className="description__content">
              <a href={`${server}/profile/${user?.username}`}>
                <FallbackImage
      ...
      {isModalVisible && (
        <Modal>
          ...
          <p className={global.text2}>
            Eliminando esta publicación, será eliminada de todas las páginas de
            la aplicación así como todos las respuestas que provengan de otros
            usuarios
          </p>
